...
def full_powerflow(r,x,ratio):
    ...
    buses = np.array([
        ...
        [3, 1, 150/1000, 100/1000, 0, 0, 1, 1.0, 0, 69, 1, 1.05, 0.85],  # Load Bus
    ])
    ...
    branches = np.array([
        [1, 2, r*ratio, x*ratio, 0, 999, 999, 999, 0, 0, 1, -360, 360],  # Impedance of 0.1 + j0.2 Ohms
        [2, 3, r*(1.0-ratio), x*(1.0-ratio), 0, 999, 999, 999, 0, 0, 1, -360, 360],  # Impedance of 0.1 + j0.2 Ohms
    ])
...
def left_powerflow(p_right,q_right,r,x):
    ...
    # Branch format: [fbus, tbus, r, x, b, rateA, rateB, rateC, ratio, angle, status, angmin, angmax]
    branches = np.array([
        [1, 2, r, x, 0, 999, 999, 999, 0, 0, 1, -360, 360],  # Impedance of 0.1 + j0.2 Ohms
    ])
...
def right_powerflow(v_mag,v_angle,r,x ):
    ...
    buses = np.array([
        ...
        [2, 1,  150/1000,  100/1000, 0, 0, 1, 1.0, 0, 69, 1, 1.05, 0.85],  # Load Bus
    ])
